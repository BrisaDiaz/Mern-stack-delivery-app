...
await act(async()=> fireEvent.blur(screen.getByTestId('lastNameInput')))    
 ...
 expect(screen.queryByText('*Apellido no valido')).not.toBeInTheDocument()
   ...
   it('diplay name error message',async () =>{
    ...
    await act(async()=>fireEvent.change(screen.getByTestId('nameInput'),{
  target:{
    value: 'brisa diaz'
  }
}))  
...
await act(async()=> fireEvent.blur(screen.getByTestId('passwordInput')))    
 ...
 expect(screen.getByText("*El largo mínimo es de 5 carácteres")).toBeInTheDocument()
...
await act(async()=> fireEvent.blur(screen.getByTestId('passwordInput')))    
 ...
 expect(screen.queryByText("*El largo mínimo es de 5 carácteres")).not.toBeInTheDocument()
...
await act(async()=> fireEvent.blur(screen.getByTestId('passwordInput')))    
 ...
 expect(screen.getByTestId('passwordInput')).toHaveValue('longsecretpasword')
...
await act(async()=> fireEvent.blur(screen.getByTestId('emailInput')))    
 ...
 expect(screen.getByTestId('emailInput')).toHaveValue('correctemailexample@gmail.com')
